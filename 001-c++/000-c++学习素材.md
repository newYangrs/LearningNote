...
- 在 main 之前运行补充 : __attribute__((constructor)) 在 main 之后运行补充 : __attribute__((destructor))
...
```
void test(int *p){　　int a=1;　　p=&a;　　cout<<p<<" "<<*p<<endl;}int main(void){    int *p=NULL;    test(p);    if(p==NULL)    cout<<"指针p为NULL"<<endl;    return 0;}//运行结果为：//0x22ff44 1//指针p为NULLvoid testPTR(int* p) {    int a = 12;    p = &a;}void testREFF(int& p) {    int a = 12;    p = a;}void main(){    int a = 10;    int* b = &a;    testPTR(b);//改变指针指向，但是没改变指针的所指的内容    cout << a << endl;// 10    cout << *b << endl;// 10    a = 10;    testREFF(a);    cout << a << endl;//12}
```
...
```c++
int *p[10]	
        		//int *p[10]表示指针数组，强调数组概念，是一个数组变量，数组大小为10，数组内每个元素				//都是指向int类型的指针变量。
int (*p)[10]
int *p(int)
int (*p)(int)
```
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibicckgeCIHxSpl7sdDhZAkvQIF8j3PQZiaQSwgMgaJaOWMiaHvqhOy0t1tJw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibicceSYFKUICMRicDIGia9TIjY3YnqB45ge3bDECbIlEXKwUibxwibSwXS1Slw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
public:   
	virtual void fun() override{        cout << " Son1::func()" <<endl;    }};class Son2 : public Base{};int main(){    Base* base = new Son1;    base->fun();    base = new Son2;    base->fun();    delete base;    base = NULL;    return 0;}// 运行结果// Son1::func()// Base::func()
```
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibiccWORUicPeJcZibiaay2XY4icyhb0yzfxbLJOEkiakrEutx95L66zDood9eGw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibiccIvy35d3lSfXYOtoOf2S7QIPXpeHPhB2sCDWgf8RXHgibgEaj9p1pTxQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
//普通；类型int a = 1, b = 3;auto c = a + b;// c为int型//const类型const int i = 5;auto j = i; // 变量i是顶层const, 会被忽略, 所以j的类型是intauto k = &i; // 变量i是一个常量, 对常量取地址是一种底层const, 所以b的类型是const int*const auto l = i; //如果希望推断出的类型是顶层const的, 那么就需要在auto前面加上cosnt//引用和指针类型int x = 2;int& y = x;auto z = y; //z是int型不是int& 型auto& p1 = y; //p1是int&型auto p2 = &x; //p2是指针类型int*
```
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibicccmTC9eDY61bINm29KgGq7gtzsibzoSWD3d6jLRqfxAUciauYw1GMvxiaA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibiccYdMoIzQkk8swibia0eTECfeibNBB7Jube7FoBPibhYzwmBTjjFMT1iaf2eg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibiccGLNSQxjcKeqXo7wCgL1cD1mN9b8eeE72vvpfQCrWWhYpv9ozPBbA2w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

大端模式中的存储方式为：

![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibicc9tuLneEBZe46vp1AwzU47icUSAT6MgCYGmBzYKx06lMG8F9F5Z2AWibg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
#include <iostream>using namespace std;//union联合体的重叠式存储，endian联合体占用内存的空间为每个成员字节长度的最大值union endian{    int a;    char ch;};int main(){    endian value;    value.a = 0x1234;    //a和ch共用4字节的内存空间    if (value.ch == 0x12)        cout << "big endian"<<endl;    else if (value.ch == 0x34)        cout << "little endian"<<endl;}
```
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbXib88YOwibeMia9Ysl29BqIibicc2AYQR1ia32QT3OP2kgFaJMqCDqn7iciapNEkyZRM79Lia5e78RNFmwlGXw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/BktAsjcTbXib88YOwibeMia9Ysl29BqIibiccjicMAXPiaXbbEdkh5GF5m9rQ6ib95HYia33Y1gMP1lCriaZlb9LZaT0655A/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX84lfZ0rGm9ia1ZzvLTT2I1uHYgRbOnKwZPbN5rBB9FuTZyO63cYg2zs8gVpv2nibhXD2HtOibXnSIJg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX84lfZ0rGm9ia1ZzvLTT2I1udpL0BDt6F9StbnKg2TJVg0d6LbOoV97nzn2axKhb0sKV4MAKgGP1kg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX84lfZ0rGm9ia1ZzvLTT2I1uBl9AV1rTBQ58w5ib3g2CZeibkvhCuzXbVTs94LiahaEaYkQ58jpNVCMaA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
#include<iostream> using namespace std; template<typename type1,typename type2>//函数模板 ype1 Max(type1 a,type2 b)  {    return a > b ? a : b; } void main()  {   cout<<"Max = "<<Max(5.5,'a')<<endl; } 
```

#### 173、智能指针出现循环引用怎么解决？

弱指针用于专门解决shared_ptr循环引用的问题，weak_ptr不会修改引用计数，即其存在与否并不影响对象的引用计数器。循环引用就是：两个对象互相使用一个shared_ptr成员变量指向对方。
...
1)  有时候类里面定义了很多int,char,struct等c语言里的那些类型的变量，我习惯在构造函数中将它们初始化为0，但是一句句的写太麻烦，所以直接就memset(this, 0, sizeof *this);将整个对象的内存全部置为0。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUphaiaXA4Wmk1kV594gKNddncFdrHInOCOHYcN3eW2bEfEOicXo5wSAKA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUQiachXLgd2HCn6vzaSB8iaqmXgO54fHXOhFIYZCwWJl29hLTbxeHaZbw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)1566573868429

**移除结点**

如果一台服务器出现问题，如上图中的nodeB，则受影响的是其逆时针方向至下一个结点之间的数据，只需将这些数据映射到它顺时针方向的第一个结点上即可，下左图

![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUbPjQZmWfT8LAvmWmGv8P2Ia8FDw5jtYkLn2PPgSjl2xrxIJU83ibPTg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)1566573901641
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUrbI9TLoGZb6Jehp7VD3skaMjPwnhjhEUqlibkTuNwiabYczpibfkB5P8A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)1566573927297
...
2)  动态编译的可执行文件需要附带一个动态链接库，在执行时，需要调用其对应动态链接库的命令。所以其优点一方面是缩小了执行文件本身的体积，另一方面是加快了编译速度，节省了系统资源。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUzJ1fibaYC8WDBsGiaNcriaO3AQqjyrVTapagI7nl6Vpib5P3as7BLA072g/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUCI670ic1ROuhRzZO77azfOMiaXed94DGib5icBPrzOlibJIvq8ibz7arHAtA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX9s2EqpTbNhbWvg2vhgEiavUicWExBAaqRhwicREPxd774VEBBICKde3fJFlqCZYnRXGxVKfBCxFga7A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)186
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRs9MAKG2pRcOkMFvnODUQbXJpicmR1ic3b5wFuQWYibj5866tUp4M9PcbUw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRshw9c8rZ3lm1c3fVoxrSYF4RtqWicu3MkHcGn5LbF8bnFJGsAACZqnQA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

##### 二级配置器

![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRs5mLnDjhtc2MLZERhdo3O4TZuG0STreeh4xITRsfYpx36cicYrYEWqfA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsNRiaKZ8k9C9DVfP00Ys4EhQoXwbIibicRgEHBTAGp0onPTX2uPliaHicBfQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsjedd1nXmrApSWQKYxcAFojkzTDdEtTUria8yiao4OS4PSqSKSNPHG0QQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsdkAQ3jpXPPYibH5WpRyQODoksFHTiaIr01RvVoUoGQPVHoUUOlhUyprw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsfGjbyFNBibtFeg1aTkDKDxERTgzibYYoxsgaUuPja6mjYCYyviaS4pwNg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsokx1IjTwozuyM76NPoMicNuHokMeY1sYh3Dz5yh7k8VEJ4CCgrQ7liaw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsfZO4cTRQsJtoyJ9EZiceqDT3ZdBwfhHQsnaRYv36lnPdDU4yB4HoYCw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsibeZG6QPEmserZVRoMVrtChN1W6Co28ohO3bJ7IuzEWF3WMFmHMtBHg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsBZ2dtS47v6V7kX3XiaicNdJxnJakLd6DMtzD0gUc04vf7HKCGO7Zdg2w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

完全二叉树内没有任何节点漏洞，是非常紧凑的，这样的一个好处是可以使用array来存储所有的节点，因为当其中某个节点位于处，其左节点必定位于![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRs4YSIoZQGmchia3KjkrSdr0bo350Mr2GZFExsBbfDqQPTZhicJ6DbzHTw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)处，右节点位于处，父节点位于（向下取整）处。这种以array表示tree的方式称为隐式表述法。
...
map支持键值的自动排序，底层机制是红黑树，红黑树的查询和维护时间复杂度均为![图片](https://mmbiz.qpic.cn/mmbiz_png/BktAsjcTbX91z2e8Z0NR1hLWmZuGXaRsUibZqDRibkLQFCXF4zYLvIHrFCYVeM804kWBDWGpkueLxtzOLHhE4zxw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)，但是空间占用比较大，因为每个节点要保持父节点、孩子节点及颜色的信息
